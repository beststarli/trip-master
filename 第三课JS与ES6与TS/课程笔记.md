...
TypeScript = JavaScript + 类型系统 + 最新的ECMAScript特性

核心特性：
- 类型系统，静态类型检查
- 编译时类型检查，运行时为纯JavaScript
- 完全兼容JavaScript，所有.js文件都是合法的.ts文件
- 开源，由微软开发维护

### interface/type
```txt
思考：type和interface定义接口有什么不同？

interface支持同名声明合并（可以多次定义同一个接口，自动合并成员，常用于扩展第三方类型或模块增强），并且类可以直接implements它，更适合定义对象形状（如React Props）

type不支持声明合并，但语法更灵活，能轻松表达联合类型、交叉类型、原始类型别名、映射类型等复杂组合，常用于工具类型或别名场景。简单来说，普通对象形状和可扩展的场景优先用interface，复杂类型组合或需要别名的场景优先用type
```
具体内容可参考[interface与type](https://www.beststarli.cn/docs/TypeScript/interface%E4%B8%8Etype)

### 接口继承/多继承
使用`extends`实现继承，多继承中间使用`,`隔开
```ts
interface Fruit {
    color: string
}
...
// 接口多继承
interface Apple extends Fruit,Shape {
    r: number
}

// 接口实现，面向接口开发
class Apple implements Fruit, Shape {
...
```txt
思考：unknown和any有什么不同?
提示：可以将any看作放弃所有类型检查，unknown需要做类型检查和断言

any 相当于完全关闭类型检查，允许对它做任何操作（随便访问属性、调用方法、随意赋值给其他类型），相当于“类型系统对我无效”，因此极不安全，现代 TypeScript 强烈不推荐日常使用

unknown 则是“安全的 any”，任何值都可以赋值给它，但反过来你不能对 unknown 类型的值做任何假设或直接操作，必须先通过类型收窄（typeof、类型守卫、类型断言等）证明它是什么类型后才能使用，因此更推荐在处理真正来源不明的外部数据（如 API 响应、用户输入）时使用 unknown，以在保持灵活性的同时强制开发者进行类型检查，避免运行时错误

any = “我不管类型，随便玩，出了问题我也不负责”
unknown = “我也不知道这是什么，但你必须先证明它是什么类型才能使用”
```
具体区别可参考[any与interface](https://www.beststarli.cn/docs/TypeScript/any%E5%92%8Cunknown)
...
### tuple
元组是一组数据的组合，通常用作**函数返回值**

元组与数组的区别：
- 元组一般是用于存储元素类型不同的数据，每个元素有自己的类型
- 数组一般用于存储同类型数据

### Union联合类型
联合类型是两个或多个不同类型组合成的类型，数值可以是联合成员中的任一类型

```ts
// 定义，CalType成为类型别名
type CalType = number | string
function calculate(num1: CalType, num2: CalType){
...
### 类型断言
由开发者告诉编译器“我比你”更清楚类型的定义，是编译阶段的“欺骗”
```ts
类型转换

const name: any = 'BestStar'
const str = name as string
```

```ts
确定可选类型的变量一定有值可使用!强制解包，实际项目开发不建议使用
...
`??`空值合并，当左侧值为`null`/`undefined`时，使用`??`后面的值

### 函数重载
示例场景：定义金额计算的方法用联合类型定义麻烦，可使用函数重载解决
```ts
// 声明
function calculate(num1: string, num2: number): number
function calculate(num1: string, num2: string): string

// 实现
function calculate(num1: any, num2: any): any {
...
// Record：构建键值对类型
type PageInfo = Record<'home' | 'about' | 'contact', {title: sting}>
```
Omit详情可参考[Omit实用类型](https://www.beststarli.cn/docs/TypeScript/Omit%E5%AE%9E%E7%94%A8%E7%B1%BB%E5%9E%8B)
